...
5. **Gerar código:** Botão para gerar o código-fonte dos analisadores e construção de compilador da linguagem projetada.
6. **Menu Layout:** Este menu permite o usuário mudar o layout da área de trabalho do Web GALS.
7. **Gerenciador de Projetos:** Opções que permitem o usuário trabalhar em múltiplos projetos (arquivos) ao mesmo tempo.
8. **Tabelas e Documentação do projeto:** Menu com opções de visualização das tabelas dos analisadores de acordo com as definições do projeto selecionado.
9. **Menu de Configurações:** abrir menu de configurações do projeto selecionado.
10. **Informações do Web GALS:** Exibir as informações do Web GALS.
11. **Espaço de Projetos**: Espaço para selecionar o projeto a ser trabalhado no WebGALS.
12. **Definições Regulares**: Área de texto para escrita das definições regulares e posterior aproveitamento na definição de Tokens.
