...
export class DelphiScannerGenerator {
  ...
  public generate(fa: FiniteAutomata, options: Options): Map<string, string> {
    ...
    } else scanner = this.buildEmptyScanner(options)
  ...
  private buildScanner(fa: FiniteAutomata, options: Options): string {
    ...
    return (
      ...
      'interface\n' +
      '\n' +
      'uses UToken, ULexicalError, UConstants' +
      inUses +
      ', SysUtils;\n' +
      '\n' +
      'type\n' +
      ...
      'function T' +
      classname +
      '.nextState(c : char; state : integer) : integer;\n' +
      'begin\n' +
      ...
      'function T' +
      classname +
      '.tokenForState(state : integer) : integer;\n' +
      'begin\n' +
      ...
      (this.lookup
        ? 'function T' +
          classname +
          '.lookupToken(base : integer; key : string) : integer;\n' +
          'var\n' +
    ...
    switch (opt.scannerTable) {
      ...
      case Options.SCANNER_TABLE_HARDCODE: {
        ...
        for (let i = 0; i < trans.size(); i++) {
          ...
          casesState.push('        ' + i + ': case integer(c) of\n')

          for (const [key, value] of m.entries()) {
            const ch: string = key
            const it: number = value
            casesState.push('            ' + ch.charCodeAt(0) + ': result := ' + it + ';\n') // TODO VALIDAR
          }

          casesState.push('            else result := -1;\n' + '        end;\n')
        }
