...
export class DelphiCommomGenerator {
  ...
  private async generateConstants(
    fa: FiniteAutomata,
    g: Grammar,
    options: Options
  ): Promise<string> {
  ...
  private lexTables(fa: FiniteAutomata, options: Options): string {
    ...
    return (
      ...
      this.context(fa) +
      (fa.specialCases.length > 0 ? this.lookup(fa) : '') +
  ...
  private scanner_error(fa: FiniteAutomata): string {
    ...
    result += '    SCANNER_ERROR : array[0..STATES_COUNT-1] of string =\n' + '    (\n'
  ...
  private scannerTable(fa: FiniteAutomata, options: Options): string {
    ...
    result += '    SCANNER_TABLE : array[0..STATES_COUNT-1, char] of integer =\n' + '    ( \n'
  ...
  private lookup(fa: FiniteAutomata): string {
    ...
    result += '    SPECIAL_CASES_KEYS : array[0..' + (count - 1) + '] of string =\n' + '        (  '
  ...
  private async transTablesLR(g: Grammar): Promise<string> {
    ...
    result +=
      '    PARSER_TABLE : array[0..' +
      (this.lrTable.length - 1) +
      ', 0..' +
      (this.lrTable[0].length - 1) +
      ', 0..1] of integer =\n'
    result += '    (\n'
