...
export class DelphiParserGenerator {
  async generate(g: Grammar, options: Options): Promise<Map<string, string>> {
  ...
  private async buildRecursiveDescendantParser(g: Grammar, options: Options): Promise<string> {
    // throws NotLLException
    const classname: string = options.parserName
    ...
    for (let symb = g.FIRST_NON_TERMINAL; symb < g.FIRST_SEMANTIC_ACTION(); symb++) {
      ...
      bfr +=
        '        else\n' +
        '            raise ESyntacticError.create(PARSER_ERROR[' +
        f.lhs +
    ...
    return (
      ...
      'interface\n' +
      '\n' +
      'uses UConstants, UToken, U' +
      scanner +
      ', U' +
      semantic +
      ', USyntacticError, UAnalysisError;\n' +
      '\n' +
      'type\n' +
      '    T' +
      classname +
  ...
  private buildLLParser(g: Grammar, options: Options): string {
    ...
    return (
      ...
      ', U' +
      semantic +
      ', USyntacticError, UAnalysisError, classes;\n' +
      '\n' +
      'type\n' +
      '    T' +
      classname +
      ...
      'function T' +
      classname +
      '.pushProduction(topStack, tokenInput : integer) : boolean;\n' +
      'var\n' +
      '    i, p, length : integer;\n' +
      'begin\n' +
      ...
      'function T' +
      classname +
      '.isNonTerminal(x : integer) : boolean;\n' +
      'begin\n' +
      ...
      'function T' +
      classname +
      '.isSemanticAction(x : integer) : boolean;\n' +
      'begin\n' +
  ...
  private generateSemanticAnalyser(options: Options): string {
    ...
    return (
      ...
      'procedure T' +
      classname +
      '.executeAction(action : integer; const token : TToken);\n' +
      'begin\n' +
      '\n' +
      'end;\n' +
      '\n' +
