...
`
  ...
  private async generateConstants(
    ...
    return (
      ...
      'pub const CASE_INSENSITIVITY: bool  = ' +
  ...
  private async syntDecls(g: Grammar, options: Options): Promise<string> {
    ...
    switch (options.parser) {
      ...
      default: {
        ...
        let result =
          `pub const FIRST_SEMANTIC_ACTION: i32 = ${g.FIRST_SEMANTIC_ACTION()};\n` +
          '\n' +
          '#[allow(nonstandard_style)]\n' +
  ...
  private async emitLLTable(g: Grammar): Promise<string> {
    ...
    result.push(
      `pub const PARSER_TABLE: [[i32; ${g.FIRST_NON_TERMINAL - 1}]; ${g.FIRST_SEMANTIC_ACTION() - g.FIRST_NON_TERMINAL}] = [\n`
    )
