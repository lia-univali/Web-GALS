...
export class PythonCommonGenerator {
  ...
  mainfunc(options: Options): string {
    ...
    return (
      ...
      (options.generateScanner
        ? `from ${pkgname}${options.scannerName} import ${options.scannerName}\n`
        : '') +
      (options.generateParser
        ? `from ${pkgname}${options.parserName} import ${options.parserName}\n`
        : '') +
      (options.generateParser
        ? `from ${pkgname}${options.semanticName} import ${options.semanticName}\n`
        : '') +
      ...
      `\ntry:\n` +
      (options.generateParser && options.generateScanner
        ? '\tsyn.parse(lex, sem)\n'
        : '\t# syn.parse(lex, sem)\n') +
  ...
  private mainfunc_lex(options: Options): string {
    switch (options.input) {
      case Options.INPUT_STREAM:
        {
          return options.generateScanner
  ...
  private async syntTables(g: Grammar, options: Options): Promise<string> {
    ...
    switch (options.parser) {
      ...
      default: //slr, lalr, lr
        return this.syntTransTable(g) + this.productionsLR(g) + this.syntErrorsLR()
    }
  }
    ...
    for (let i = 0; i < pl.size(); i++) {
      ...
      if (rhs.length > 0) {
        ...
        for (let j = 0; j < rhs.length; j++) {
          ...
          if (productions[i][j].length > max) max = productions[i][j].length
        }
      } else {
