...
export class Grammar {
  ...
  public LAST_SEMANTIC_ACTION(): number {
    return this.FIRST_SEMANTIC_ACTION() + this.SEMANTIC_ACTION_COUNT
  }
  ...
  private setProductions(productions: List<Production>) {
    ...
    for (let i = 0; i < this._productions.size(); i++) {
      ...
      for (let j = 0; j < this._productions.get(i).get_rhs().length; j++)
        if (this._productions.get(i).get_rhs()[j] > max) max = this._productions.get(i).get_rhs()[j]
    }
  ...
  public asNormalLR(): Grammar {
    ...
    const production: Production = new Production(
      null,
      this.FIRST_SEMANTIC_ACTION() + newSymbols - 1,
      [this.startSymbol]
    )
  ...
  public createProduction(lhs: number, rhs?: number[]): Production | null {
    ...
    for (let i = 0; i < this._productions.size(); i++)
      if (this._productions.get(i).equals(p)) return null
  ...
  public isEpsilon(x: number[], start?: number): boolean {
    ...
    for (let i = start; i < x.length; i++) if (!this.isSemanticAction(x[i])) return false
    return true
  }
  ...
  private markEpsilon(): OrderedIntegerSet {
    ...
    for (let i = 0; i < this._productions.size(); i++) {
      ...
      if (this.isEpsilon(production.get_rhs())) result.add(production.get_lhs())
    }
    for (let i = this.FIRST_SEMANTIC_ACTION(); i <= this.LAST_SEMANTIC_ACTION(); i++) result.add(i)
  ...
  public first(x: number[] | number, start?: number): OrderedIntegerSet {
    if (!Array.isArray(x)) {
      if (this.isSemanticAction(x)) return Grammar.EMPTY_SET
      else return this.firstSet[x]
    }
    ...
    if (this.isEpsilon(x, start)) {
      ...
      while (this.isSemanticAction(x[start])) start++
  ...
  private fillFirstSet() {
    ...
    for (let A = this.FIRST_NON_TERMINAL; A < this.FIRST_SEMANTIC_ACTION(); A++) {
      if (derivesEpsilon.has(A)) this.firstSet[A].add(Grammar.EPSILON)
    }
    ...
    for (let a = Grammar.FIRST_TERMINAL; a < this.FIRST_NON_TERMINAL; a++) {
      ...
      for (let A = this.FIRST_NON_TERMINAL; A < this.FIRST_SEMANTIC_ACTION(); A++) {
        ...
        if (exists) this.firstSet[A].add(a)
      }
    }
        ...
        for (let j = 0; j < P.get_rhs().length; j++) {
          ...
          if (this.isNonTerminal(P.get_rhs()[j])) {
            ...
            if (P.get_rhs().length > j + 1) {
              ...
              if (!changes && !this.followSet[P.get_rhs()[j]].equals(old)) changes = true
            }
  ...
  public stringFirstFollow(): string {
    ...
    for (let i = this.FIRST_NON_TERMINAL; i < this.firstSet.length; i++) {
      ...
      for (let j = 0; j < this.firstSet[i].size; j++) {
        if (this.firstSet[i].list()[j]) bfr += `${this.symbols[j]} `
      }
      bfr += '}'
      result = +bfr + '\n'
    }
    for (let i = this.FIRST_NON_TERMINAL; i < this.followSet.length; i++) {
      ...
      for (let j = 0; j < this.followSet[i].size; j++) {
        if (this.followSet[i].list()[j]) bfr += this.symbols[j] + ' '
      }
      bfr += '}'
      result += bfr + '\n'
    }
   ...
   */
  public removeImproductiveSymbols() {
    //TODO: throws EmptyGrammarException
  ...
  public productionsFor(symbol: number): OrderedIntegerSet {
    ...
    for (let i = 0; i < this.productions.size(); i++) {
      if (this.productions.get(i).get_lhs() == symbol) result.add(i)
    }
    return result
  }
        ...
        for (let it = 0; it < prodList.size(); it++) {
          ...
          if (P.get_lhs() == i && P.firstSymbol() == j) {
            ...
            for (let it2 = 0; it2 < prodList.size(); it2++) {
              ...
              if (P2.get_lhs() == j) {
                const rhs: number[] = new Array<number>(
                  P2.get_rhs().length + P.get_rhs().length - 1
                )
                let k = 0
                for (; k < actions.length; k++) rhs[k] = actions[k]
                let m = k
                for (k = 0; k < P2.get_rhs().length; k++) rhs[k + m] = P2.get_rhs()[k]
                m = m + k - (actions.length + 1)
                for (k = actions.length + 1; k < P.get_rhs().length; k++)
                  rhs[k + m] = P.get_rhs()[k]

                const newProduction = this.createProduction(P.get_lhs(), rhs)
                if (newProduction != null) prodList.add(newProduction)
              }
            }
          }
        }
  ...
  private removeDirectRecursion() {
    for (let i = this.FIRST_NON_TERMINAL; i < this.FIRST_SEMANTIC_ACTION(); i++) {
      ...
      if (newSymbol != -1) {
        ...
        if (producton != null) this.productions.add(producton)
      }
    }
    this.fillFirstSet()
    this.fillFollowSet()
  ...
  private createSymbol(s: string): number {
    for (const p of this._productions) {
      ...
      for (let j = 0; j < rhs.length; j++)
        if (this.isSemanticAction(rhs[j])) rhs.push(j, rhs[j] + 1)
    }
        ...
        if (p.get_rhs().length == 1) {
          ...
          for (let j = 0; j < rhs.length; j++) {
            if (rhs[j] == b) {
              let allEpsilon = true
              for (let k = 0; k < j; k++) {
                if (!derivesEpsilon.list()[rhs[k]]) allEpsilon = false
              }
              for (let k = j + 1; k < rhs.length; k++) {
                if (!derivesEpsilon.list()[rhs[k]]) allEpsilon = false
              }
              if (allEpsilon) return true
            }
          }
        }
      }
  ...
  public removeUnitaryProductions() {
    ...
    for (let i = 0; i < this._productions.size(); i++) {
      const p: Production = this._productions.get(i)
      if (p.get_rhs().length != 1 || p.get_rhs()[0] != p.get_lhs()) prods.add(p)
    }
      ...
      if (p.get_rhs().length != 1 || !this.isNonTerminal(p.get_rhs()[0])) {
        for (let j = this.FIRST_NON_TERMINAL; j < N.length; j++) {
          if (N[j].list()[p.get_lhs()]) {
            ...
            if (np != null) this._productions.add(np)
          }
        }
      }
    }
      ...
      if (!this.isEpsilon(p.get_rhs())) {
        ...
        while (i < p.get_rhs().length) {
          ...
          for (; i < p.get_rhs().length; i++) {
            if (!this.isSemanticAction(p.get_rhs()[i]) && E.list()[p.get_rhs()[i]]) break
          }
          if (i < p.get_rhs().length) {
            const pNew: Production | null = this.derivationAt(p, i)
            if (pNew != null && !prods.contains(pNew)) prods.add(pNew)
            i++
          }
        }
      }
  ...
  private derivationAt(p: Production, index: number): Production | null {
    ...
    for (let k = 0; k < this._productions.size(); k++) {
      if (
        this._productions.get(k).get_lhs() == p.get_rhs()[index] &&
    ...
    for (let k = 0; k < index; k++) rhs.push(p.get_rhs()[k])
    for (let k = 0; k < rhsP.length; k++) rhs.push(rhsP[k])

    for (let k = index + 1; k < p.get_rhs().length; k++) rhs.push(p.get_rhs()[k])
  ...
  public sort() {
    for (let i = this.FIRST_NON_TERMINAL; i < this.FIRST_SEMANTIC_ACTION(); i++) {
      ...
      let j = i + 1
      for (; j < this.FIRST_SEMANTIC_ACTION(); j++) if (this._symbols[j] == s) break
      if (j < this.FIRST_SEMANTIC_ACTION()) {
        //achou
        const to = i + 1,
          from = j
  ...
  private moveSymbol(from: number, to: number) {
    ...
    for (let k = from; k > to; k--) this._symbols[k] = this._symbols[k - 1]
    this._symbols[to] = s

    if (this._startSymbol == from) this._startSymbol = to
    else if (this._startSymbol >= to && this._startSymbol < from) this._startSymbol++

    for (const p of this._productions) {
      if (p.get_lhs() == from) p.set_lhs(to)
      else if (p.get_lhs() >= to && p.get_lhs() < from) p.set_lhs(p.get_lhs() + 1)
      const rhs: number[] = p.get_rhs()
      for (let k = 0; k < rhs.length; k++) {
        if (rhs[k] == from) rhs.push(k, to)
        else if (rhs[k] >= to && rhs[k] < from) rhs.push(k, rhs[k] + 1)
      }
    }
  }
   ...
   */
  public isLL(): boolean {
    return this.isFactored() && !this.hasLeftRecursion() && this.passThirdCondition()
  }
  ...
  private getProductiveSymbols(): OrderedIntegerSet {
    ...
    for (let i = Grammar.FIRST_TERMINAL; i < this.FIRST_NON_TERMINAL; i++) SP.add(i)

    for (let i = this.FIRST_SEMANTIC_ACTION(); i <= this.LAST_SEMANTIC_ACTION(); i++) SP.add(i)
      ...
      for (let i = this.FIRST_NON_TERMINAL; i < this.FIRST_SEMANTIC_ACTION(); i++) {
        if (!SP.has(i)) {
          for (let j = 0; j < this._productions.size(); j++) {
            ...
            if (P.get_lhs() == i) {
              let pass = true
              for (let k = 0; k < P.get_rhs().length; k++) pass = pass && SP.has(P.get_rhs()[k])
              if (pass) {
                Q.add(i)
                change = true
              }
  ...
  public uselessSymbolsHTML(): string {
    ...
    if (count == 0) result += 'Não há símbolos inúteis'
  ...
  public setToStr(b: OrderedIntegerSet): string {
    ...
    for (let j = 0; j < b.size; j++) {
      if (b.list()[j]) bfr += '"' + this._symbols[j] + '" '
    }
    bfr += '}'
    return bfr
  }
  ...
  public factorate() { //throws LeftRecursionException
    if (this.hasLeftRecursion()) throw Error('new LeftRecursionException();') //new LeftRecursionException();
  ...
  private factorateLeft(symb: number): boolean {
    ...
    if (!conflict.isEmpty()) {
      ...
      for (let i = 0; i < this._productions.size(); i++) {
        ...
        if (
          p.get_lhs() == symb &&
          this.first(p.get_rhs()).list()[confictSymbol] &&
      ...
      for (const it of conflict.list()) {
        ...
        p.set_lhs(newIndex)
        if (p.get_rhs().length > prefix.length) p.get_rhs().splice(0, prefix.length)
        // p.rhs.length == prefix.length
        else p.clear_rhs()
      }
      const rhs = new Array<number>()
      rhs.push(...prefix)
  ...
  public leftMostDerive(p: Production): List<Production> {
    if (this.isTerminal(p.firstSymbol())) return new List<Production>()
    else {
      ...
      for (const it of this.productionsFor(symb).list()) {
        ...
        const rhs = new Array<number>()
        for (let i = 0; i < actions.length; i++) rhs.push(actions[i])
        for (let i = 0; i < p1.get_rhs().length; i++) rhs.push(p1.get_rhs()[i])
        for (let i = actions.length + 1; i < p.get_rhs().length; i++) rhs.push(p.get_rhs()[i])

        const n: Production | null = this.createProduction(p.get_lhs(), rhs)
        if (n != null && !newProds.contains(n)) newProds.add(n)
      }
      return newProds
    }
  }
    ...
    do {
      ...
      if (pro.get_rhs().length > index) {
        ...
        for (; it > prods.size; it++) {
          ...
          if (p.get_rhs().length <= index || p.get_rhs()[index] != s) repeat = false
        }
        if (repeat) {
  ...
  private conflict(prods: OrderedIntegerSet, result: OrderedIntegerSet): number {
    ...
    for (const it of prods) {
      ...
      for (const i of this.first(p.get_rhs())) symbs[i]++
    }
    ...
    if (max > 1) {
      ...
      for (const pos of prods) {
        if (this.first(this._productions.get(pos).get_rhs()).list()[indexMax]) result.add(pos)
      }
    }
  ...
  public toString(): string {
    ...
    for (let i = 0; i < this.productions.size(); i++) {
      ...
      if (!(this._symbols[P.get_lhs()] == lhs)) {
        ...
        bfr += '\n'
        for (let j = 0; j < lhs.length; j++) bfr += ' '
        bfr += '   |'
      }
  ...
  public clone(): Grammar {
    try {
      ...
      for (let i = 0; i < T.length; i++) T[i] = this._symbols[i + 2].toString()
      ...
      for (let i = 0; i < this._productions.size(); i++) {
        ...
        for (let j = 0; j < rhs.length; j++) rhs[j] = this._productions.get(i).get_rhs()[j]
  ...
  private removeSymbol(s: number) {
    ...
    if (this._startSymbol > s) this._startSymbol--
    if (this.FIRST_NON_TERMINAL > s) this.FIRST_NON_TERMINAL--
    ...
    for (let i = 0; i < this._productions.size(); i++) {
      ...
      if (p.get_lhs() == s) {
        ...
        continue
      } else if (p.get_lhs() > s) p.set_lhs(p.get_lhs() - 1)
      ...
      for (let j = 0; j < p.get_rhs().length; j++) {
        ...
        if (p.get_rhs()[j] > s) p.set_rhs(j, p.get_rhs()[j] - 1)
      }
    }
  }
   ...
   * @throws EmptyGrammarException se o símbolo inicial for removido
   */
  private updateSymbols(keep: OrderedIntegerSet) {
    //throws EmptyGrammarException
  ...
  public id_for_production(p: Production): number | null {
    if (this._productions.contains(p)) return this._productions.indexOf(p)
    else return null
  }
