...
export class LRItem {
  ...
  public equals(obj: LRItem): boolean {
    try {
      ...
      return (
        obj.production.equals(this._production) &&
  ...
  public toString(): string {
    ...
    for (let i = 0; i < this._production.get_rhs().length && i < this._position; i++) {
      ...
      if (this._g.isSemanticAction(s)) bfr += '#' + (s - this._g.FIRST_SEMANTIC_ACTION()) + ' '
      else bfr += this._g.symbols[s] + ' '
    }
  ...
  public compareTo(it: LRItem): number {
    ...
    if (cmp != 0) return cmp
      ...
      if (cmp != 0) return cmp
      else return this._lookahead - it.lookahead
    }
  }
}
