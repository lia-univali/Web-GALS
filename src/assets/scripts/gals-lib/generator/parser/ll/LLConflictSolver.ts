...
export class LLConflictSolver extends ConflictSolver {
  ...
  async resolve(g: Grammar, input: number): Promise<number> {
    ...
    if (input == 0) inText = '$'
    else inText = g.terminals[input - 1]
    ...
    else if (resultNumber < 1 || resultNumber > conflict.size) resultNumber = 1

    return (
      conflict.get(resultNumber) ??
      (() => {
        throw new Error(`Opção não encontrada de conflito: ${resultNumber}`)
      })()
    )
  }
