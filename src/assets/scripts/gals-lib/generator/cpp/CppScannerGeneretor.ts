...
export class CppScannerGeneretor {
  ...
  private buildScannerCpp(fa: FiniteAutomata, options: Options): string {
    ...
    if (!this.sensitive) result += '#include <cctype>\n\n'
    ...
    const funcs =
      ...
      'int ' +
      classname +
      '::nextState(unsigned char c, int state) const\n' +
      '{\n' +
      ...
      (this.lookup
        ? 'TokenId ' +
          classname +
          '::lookupToken(TokenId base, const std::string &key)\n' +
          '{\n' +
