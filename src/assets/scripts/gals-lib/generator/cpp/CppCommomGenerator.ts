...
export class CppCommomGenerator {
  ...
  private async generateConstantsH(
    fa: FiniteAutomata,
    g: Grammar,
    options: Options
  ): Promise<string> {
  ...
  private constList(fa: FiniteAutomata, g: Grammar): string {
    ...
    if (fa != null) tokens = fa.tokens.toArray()
    else if (g != null) tokens = g.terminals
    else throw new Error('Erro Interno')
  ...
  private lexDecls(fa: FiniteAutomata, options: Options): string {
    ...
    return (
      ...
      '\n' +
      (fa.hasContext() ? 'extern int SCANNER_CONTEXT[STATES_COUNT][2];\n' + '\n' : '') +
      (fa.specialCases.length > 0
        ? 'extern int SPECIAL_CASES_INDEXES[' +
          (fa.getSpecialCasesIndexes().length + 1) +
          '];\n' +
          '\n' +
          'extern const char *SPECIAL_CASES_KEYS[' +
          fa.specialCases.length +
          '];\n' +
          '\n' +
          'extern int SPECIAL_CASES_VALUES[' +
          fa.specialCases.length +
          '];\n' +
          '\n'
        : '') +
    ...
    switch (options.parser) {
      ...
      case Options.PARSER_LL: {
        ...
        return (
          ...
          ';\n' +
          '\n' +
          'const int FIRST_NON_TERMINAL    = ' +
          g.FIRST_NON_TERMINAL +
          ';\n' +
          'const int FIRST_SEMANTIC_ACTION = ' +
          g.FIRST_SEMANTIC_ACTION() +
          ';\n' +
          '\n' +
          'extern int PARSER_TABLE[' +
          numNT +
          '][' +
          (g.FIRST_NON_TERMINAL - 1) +
          '];\n' +
          '\n' +
          ...
          '];\n' +
          '\n' +
          'extern const char *PARSER_ERROR[' +
          (g.FIRST_NON_TERMINAL + numNT) +
          '];\n' +
          '\n'
        )
      }
      default: {
        ...
        return (
          ...
          'const int ERROR  = 5;\n' +
          '\n' +
          'extern const int PARSER_TABLE[' +
          this.lrTable.length +
          '][' +
          this.lrTable[0].length +
          '][2];\n' +
          '\n' +
          'extern const int PRODUCTIONS[' +
          g.productions.size() +
          '][2];\n' +
          '\n' +
          'extern const char *PARSER_ERROR[' +
  ...
  private lexTables(fa: FiniteAutomata, options: Options): string {
    ...
    for (let i = 0; i < count; i++) {
      ...
      for (let j = n.length; j < max; j++) result += ' '
      result += n + ', '
    }
    result = result.slice(0, -2)
    result += ' };\n\n'
    ...
    result += 'const char *SCANNER_ERROR[STATES_COUNT] =\n' + '{\n'
    ...
    for (let i = 0; i < count; i++) {
      ...
      for (let j = 0; j < error.length; j++) {
        if (error.charAt(j) == '"') result += '\\"'
        else result += error.charAt(j)
      }
  ...
  private scannerTable(fa: FiniteAutomata, options: Options): string {
    if (options.scannerTable == Options.SCANNER_TABLE_HARDCODE) return ''
  ...
  private specialCases(fa: FiniteAutomata): string {
    if (fa.specialCases.length > 0) {
      ...
      result += 'int SPECIAL_CASES_INDEXES[' + (indexes.length + 1) + '] =\n' + '    { '
      ...
      count = sc.length
      result += 'const char *SPECIAL_CASES_KEYS[' + count + '] =\n' + '    { '
      ...
      result += 'int SPECIAL_CASES_VALUES[' + count + '] =\n' + '    { '
  ...
  private async syntTables(g: Grammar, options: Options): Promise<string> {
    // TODO throws NotLLException
    if (g == null) return ''
    ...
    switch (options.parser) {
      ...
      case Options.PARSER_LL:
        return (
          (await this.syntTransTable(new LLParser(g))) +
          this.productionsLL(g) +
          this.syntErrorsLL(g)
        )
      default: //slr, lalr, lr
        return (await this.syntTransTable(g)) + this.productionsLR(g) + this.syntErrorsLR()
    }
  }
  ...
  private syntTransTableGrammar(g: Grammar): string {
    ...
    result +=
      'const int PARSER_TABLE[' + this.lrTable.length + '][' + this.lrTable[0].length + '][2] =\n'
    result += '{\n'

    let max = this.lrTable.length
    if (g.productions.size() > max) max = g.productions.size()
    ...
    for (let i = 0; i < this.lrTable.length; i++) {
      ...
      for (let j = 0; j < this.lrTable[i].length; j++) {
        ...
        for (let k = str.length; k < max; k++) result += ' '
        result += str + '},'
      }
  ...
  private async syntTransTableLL(g: LLParser): Promise<string> {
    ...
    for (let i = 0; i < table.length; i++) {
      ...
      for (let j = 0; j < table[i].length; j++) {
        bfr += ' '
        for (let k = table[i][j].length; k < max; k++) bfr += ' '
        bfr += table[i][j] + ','
      }
      bfr = bfr.slice(0, -1)
      bfr += ' },\n'
  ...
  private productionsLL(g: Grammar): string {
    ...
    for (let i = 0; i < pl.length; i++) {
      ...
      if (rhs.length > longest) longest = rhs.length
      if (rhs.length > 0) {
        productions[i] = []
        productions[i][0] = rhs.length.toString()
        for (let j = 0; j < rhs.length; j++) {
          productions[i][j + 1] = rhs[j].toString()
          if (productions[i][j + 1].length > max) max = productions[i][j + 1].length
        }
      } else {
        productions[i] = []
        productions[i][0] = '1'
    ...
    for (let i = 0; i < productions.length; i++) {
      ...
      for (let j = 0; j < productions[i].length; j++) {
        bfr += ' '
        for (let k = productions[i][j].length; k < max; k++) bfr += ' '
        bfr += productions[i][j] + ','
      }
      for (let j = productions[i].length; j <= longest; j++) {
        bfr += ' '
        for (let k = 1; k < max; k++) bfr += ' '
        bfr += '0' + ','
      }
      bfr = bfr.slice(0, -1)
      bfr += ' },\n'
  ...
  private syntErrorsLR(): string {
    ...
    result += 'const char *PARSER_ERROR[' + this.lrTable.length + '] =\n' + '{\n'

    for (let i = 0; i < this.lrTable.length; i++) result += '    "Erro estado ' + i + '",\n'
