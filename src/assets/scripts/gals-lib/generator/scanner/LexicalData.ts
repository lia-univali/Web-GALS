...
class SpecialCaseValue{ // Using static in the original code
...
export class LexicalData{
	...
	private _specialCasesValues: Map<string | null,  SpecialCaseValue>  = new Map<string | null, SpecialCaseValue>;
    ...
    public addIgnore(ignore: string){
        if (this._ignore.length > 0)
            this._ignore = this.ignore+"|"+ignore;
        else
            this._ignore = ignore;
    }
