...
export class JavaCommonGenerator {
  ...
  private generateScannerConstants(fa: FiniteAutomata | null, options: Options): string {
    ...
    result.push('public interface ScannerConstants\n' + '{\n')
  ...
  private async generateParserConstants(g: Grammar | null, options: Options): Promise<string> {
    ...
    result.push('public interface ParserConstants\n' + '{')
  ...
  private genLexTables(fa: FiniteAutomata, options: Options): string {
    ...
    return (
      ...
      (fa.hasContext() ? '\n' + this.context(fa) : '') +
      '\n' +
      (fa.specialCases.length > 0 ? this.special_cases(fa) + '\n' : '') +
  ...
  private scanner_error(fa: FiniteAutomata): string {
    ...
    result.push('    String[] SCANNER_ERROR =\n' + '    {\n')
    ...
    for (let i = 0; i < count; i++) {
      ...
      for (let j = 0; j < error.length; j++) {
        if (error.charAt(j) == '"') result.push('\\"')
        else result.push(error.charAt(j))
      }
  ...
  private emitLRTable(g: Grammar): string {
    ...
    for (let i = 0; i < tbl.length; i++) {
      ...
      for (let j = 0; j < tbl[i].length; j++) {
        ...
        for (let k = str.length; k < max; k++) result.push(' ')
  ...
  private emitModifiedLRTable(g: Grammar): string {
    ...
    result.push('    {\n')
    result.push(
      '        int table[][][] = new int[' + tbl.length + '][' + tbl[0].length + '][2];\n'
    )
    result.push('\n')
  ...
  private async genLLSyntTables(g: Grammar, type: number): Promise<string | null> {
    ...
    if (type == Options.PARSER_LL) {
      ...
      return result.join('')
    } else if (type == Options.PARSER_REC_DESC) return this.emitErrorTableLL(g)
    else return null
  }
  ...
  private lex_table(fa: FiniteAutomata): string {
    ...
    for (let i = 0; i < count; i++) {
      ...
      for (let c = 0; c < 256; c++) {
        ...
        for (let j = n.length; j < max; j++) result.push(' ')
        result.push(n)
        result.push(', ')
      }
      result.pop()
  ...
  private special_cases(fa: FiniteAutomata): string {
    ...
    result.push('    int[] SPECIAL_CASES_INDEXES =\n' + '        { ')
    ...
    result.push('    String[] SPECIAL_CASES_KEYS =\n' + '        {  ')
    count = sc.length
    for (let i = 0; i < count; i++) {
      result.push('"')
    ...
    result.push('    int[] SPECIAL_CASES_VALUES =\n' + '        {  ')
    count = sc.length
    for (let i = 0; i < count; i++) {
  ...
  private emitErrorTableLR(): string {
    ...
    result.push('    String[] PARSER_ERROR =\n' + '    {\n')
