...
export class Scanner implements BasicScanner {
    ...
    while (this.hasMoreChars()) {
      ...
      switch (c) {
        ...
        case '/': {
          ...
          if (this.returnComents) return t
          else continue
        }
  ...
  private analyseComent(): Token {
    // throws LexicalError
    const start = this.pos - 1
    if (!this.hasMoreChars()) throw new LexicalError("Caracter Inválido: '/'", start)
  ...
  private analyseDerives(): Token {
    ...
    if (this.input.length - start >= 3) {
      ...
      if (c == ':') {
        c = this.nextChar()
        if (c == '=') return new Token(Constants.DERIVES, '::=', start)
      }
    }
  ...
  private analyseTerminal(c: string): Token {
    // throws LexicalError
    const start = this.pos - 1
    let bfr = ''
    bfr += c
    if (c == '"') {
      ...
      if (bfr.length == 0 || !close) throw new LexicalError('Terminal inválido', start)
    } else {
      while (this.hasMoreChars()) {
        c = this.nextChar()
  ...
  private analyseNonTerminal(): Token {
    //throws LexicalError
    const start = this.pos - 1
    let bfr = ''
    let c = '<'
    ...
    if (bfr.length == 0 || c != '>') throw new LexicalError('Não-Terminal inválido', start)
    else return new Token(Constants.NON_TERM, '<' + bfr + '>', start)
  }

  private analyseAction(): Token {
    //throws LexicalError
    const start = this.pos - 1
    ...
    if (bfr.length == 0) throw new LexicalError('Ação Semântica inválida', start)
    else return new Token(Constants.ACTION, bfr.toString(), start)
  }
  ...
  private nextChar() {
    if (this.hasMoreChars()) return this.input.charAt(this.pos++)
