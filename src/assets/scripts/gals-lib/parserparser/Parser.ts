...
export class Parser {
  ...
  public parse(t: List<string>, nt: List<string>, prods: string): Grammar {
    ...
    try {
      for (let i = 0; i < t.size(); i++) {
        ...
        if (token == null) {
          ...
          if (s.has(str)) throw new SemanticError('Terminal repetido : ' + str, token.position)
          else s.add(str)

          t.set(i, str) // TODO test if behavior is correct
          map.set(str, pos)
          pos++
          if ((token = scanner.nextToken()) != null) {
            // mais de um terminal por linha
            throw new SemanticError(
              'Cada linha deve conter a declaração de apenas um símbolo terminal',
              token.position
            )
          }
        }
      }
      if (t.size() == 0) throw new SemanticError('Conjunto de Terminais não pode ser vazio', 0)
    } catch (e) {
    ...
    try {
      for (let i = 0; i < nt.size(); i++) {
        ...
        if (token == null) {
          ...
          if (s.has(str)) throw new SemanticError('Não-terminal repetido : ' + str, token.position)
          else s.add(str)
          ...
          if ((token = scanner.nextToken()) != null) {
            // mais de um terminal por linha
            throw new SemanticError(
              'Cada linha deve conter a declaração de apenas um símbolo não-terminal',
              token.position
            )
          }
        }
      }
      if (nt.size() == 0) throw new SemanticError('Conjunto de Não-Terminais não pode ser vazio', 0)
    } catch (e) {
  ...
  public step(): boolean {
    ...
    if (this.currentToken == null) a = Constants.DOLLAR
    else a = this.currentToken.id
    ...
    if (x == Constants.EPSILON) {
      ...
      if (x == a) {
        ...
        throw new SyntacticError(
          'Era esperado ' + Constants.EXPECTED_MESSAGE[x],
      ...
      if (p >= 0) {
        ...
        throw new SyntacticError(
          Constants.PARSER_ERROR[x - Constants.FIRST_NON_TERMINAL],
          this.scanner.getPosition()
        )
      }
  ...
  /**
   * @return TRUE se x é um símbolo terminal
   */
  ...
  /**
   * @return TRUE se x é um símbolo não terminal
   */
  ...
  /**
   * @return TRUE se x é uma Ação Semântica
   */
