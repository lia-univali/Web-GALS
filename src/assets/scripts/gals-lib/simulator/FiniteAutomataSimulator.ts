...
export class FiniteAutomataSimulator implements BasicScanner {
  ...
  nextToken(): Token | null {
    ...
    if (ctxtState != -1 && this._fa.getOrigin(endState) == ctxtState) end = ctxtEnd
    this._position = end
  ...
  private nextChar(): string {
    if (this.hasInput()) return this._input.charAt(this._position++)
