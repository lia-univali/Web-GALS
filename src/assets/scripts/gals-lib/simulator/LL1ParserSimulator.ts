...
// 		let p: List<Production> = this.grammar.productions;
// 		this.productions = [];
// 		for (let i=0; i< p.size(); i++)
// 		{
// 			let rhs: number[] = p.get(i).get_rhs();
// 			if (rhs.length > 0)
// 			{
// 				this.productions[i] = [];
...
// 		{
// 			let p = this.table[x-this.FIRST_NON_TERMINAL][a-1] //this.table[x-FIRST_NON_TERMINAL][a-1];
// 			if (p != -1)
// 			{	
// 				let production: number[] = this.productions[p];
// 				//empilha a produção em ordem reversa
// 				for (let i = production.length - 1; i >= 0; i--)
// 				{
// 					this.stack.push(production[i]);
// 				}
// 				let n: TreeNode<string> = new TreeNode(this.symb[x]);
// 				this.node.add(n);
// 				this.node = n;
...
// 	public parse(scnr: BasicScanner, root: TreeNode<any>)  //throws LexicalError, SyntaticError, SemanticError
// 	{			
// 		this.scanner = scnr;
// 		this.node = root;
