...
export class LL1ParserSimulator {
  ...
  public constructor(table: number[][], parser: LLParser) {
    ...
    for (let i = 0; i < p.size(); i++) {
      ...
      if (rhs.length > 0) {
        this.productions[i] = []
        for (let j = 0; j < rhs.length; j++) this.productions[i][j] = rhs[j]
      } else this.productions[i] = [0]
    }
  ...
  public step(): boolean {
    ...
    if (x == LL1ParserSimulator.EPSILON) {
      ...
      while (itg == 1) {
        ...
        this.node = parent
        if (this.nodeCount.size() > 0) itg = this.nodeCount.pop()
        else break
      }
  ...
  public parse(scnr: BasicScanner, root: TreeNode<string>): TreeNode<string> {
    //throws LexicalError, SyntacticError, SemanticError
    this.scanner = scnr
    this.node = root
