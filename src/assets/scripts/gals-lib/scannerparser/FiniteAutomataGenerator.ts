...
export class FiniteAutomataGenerator {
    ...
    private _specialCases: HashMap<number, Map<string, number>> =  new HashMap<number, Map<string, number>> ;
    ...
    public addDefinition( id: string, root: Node){ // throws SemanticError
    ...
    public addExpression(id: string, root: Node,  backtrack: boolean){ //throws SemanticError
    ...
    public addSpecialCase(id: string, base: string, value: string){ //throws SemanticError
    ...
    public generateAutomata(): FiniteAutomata | null{ // TODO throws SemanticError
    ...
    public makeAtomata (states: List<Set<number>>, trans: Map<number, Map<string, number>>, finals: Map<number, number>,
        back: Map<number, boolean>, context: Map<number, number> ): FiniteAutomata{ //throws SemanticError
        ...
        for (let i = 0; i < fin.length; i++) { // verify if length does the same
        ...
        return new FiniteAutomata(this._alphabet, transitions, fin, scIndexes, sc, cont, this._tokenList, this._sensitive);	
    }
    ...
    private getPositionStates(states: List<IntegerSet>, setParam: IntegerSet): number{
        ...
        for(let set of states){
            ...
            let equal: boolean = setOrdered.length === setParamOrdered.length && setOrdered.every((element, index) => element === setParamOrdered[index]);
...
gathering:  {
    ...
    private computeNext(){// is computeNext() in Java
    ...
    private computeNextNode(root: Node | null){
        ...
        if(root.left != null)
            this.computeNextNode(root.left);

        if(root.right != null)
            this.computeNextNode(root.right);
    }
