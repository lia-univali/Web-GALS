...
export class Node {
    ...
    constructor(id: number, left: Node | null, right: Node | null) {
        ...
        if (left != null)
            this.alphabet.addAll(left.alphabet);

            
        if (right != null)
			this.alphabet.addAll(right.alphabet);
    ...
    public static createComplementNode(node: Node): Node{
        ...
        if (!node.alphabet.has('\n'.charCodeAt(0)))
            newNode.alphabet.add('\n'.charCodeAt(0));

        if (!node.alphabet.has('\r'.charCodeAt(0)))
            newNode.alphabet.add('\r'.charCodeAt(0));
