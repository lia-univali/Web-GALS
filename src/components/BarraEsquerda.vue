    ...
    methods: {
        ...
        abrirArquivo() {
            ...
            reader.onload = function () {
                ...
                const splitResultado: string[] = (reader.result as string).split(/#Options\n|\n#RegularDefinitions\n|\n#Tokens\n|\n#NonTerminals\n|\n#Grammar\n/);
            ...
            <div v-if="paginaAberta =='Projetos'" class="abaProjetos">
                <div class="lista__projetos" v-if="store.totalProjetos > 0">
                    ...
                    <div v-for="projeto in projetos" :key="projeto.id" class="projeto__acaoes">
                        <button @click="store.changeSelected(projeto.id)"
                            class="botao__mudar__projeto">{{projeto.fileName}}</button>
                        <button @click="store.deleteProject(projeto.id)" class="botao__excluir__projeto">X</button>
                    </div>
...
</style>@/assets/scripts/salvador@/assets/scripts/saver